...
async def select_child(device_name: str, child_id: str):
    db = get_db()
...
async def get_children_for_device(device_name: str):
    db = get_db()
    device = await db.devices.find_one({"device_name": device_name})
    if not device:
        raise HTTPException(404, "Device not found")
...
async def get_active_child_profile(device_name: str):
    db = get_db()
    device = await db.devices.find_one({"device_name": device_name})
    if not device:
        raise HTTPException(404, "Device not found")
...
async def verify_child_pin(device_name: str, child_id: str, child_pin: str):
    db = get_db()
    device = await db.devices.find_one({"device_name": device_name})
    if not device:
        raise HTTPException(404, "Device not found")
